# -*- coding: utf-8 -*-
"""checks.py —— 六关自测。补全 tool_backend.py 的 TODO 后逐关跑：python checks.py 1"""
import sys, json
...
def verify_citations(cited_ids, clause_ctx):
    ...
    # 【关卡留白 TODO-3】实现：遍历 cited_ids，clause 类 id 必须在 real 中；policy:/fact: 前缀视为合法。
    #   提示：return all(cid in real or cid.split(':')[0] in ('policy','fact') for cid in cited_ids)
    return True   # ← 替换为真正的校验（当前恒真 = 拦不住幻觉）
