# -*- coding: utf-8 -*-
"""
[参考答案 SOLVED] tool_backend_solved.py —— 8 个 function-calling 工具的参考实现（对接 Neo4j + seed.json）
...
def entity_get(id, fields=None, _profile="subject-due-diligence"):
    ...
    for f in want:
        # 【SOLVED · TODO-1 切片权限边界】只返回切片允许的字段，越界进 forbidden
        if f in allowed:
            out[f] = rec.get(f)
        else:
            forbidden.append(f)
...
def graph_traverse(startId, rel, hops, direction="out"):
    ...
    # 【SOLVED · TODO-2 多跳穿透 + 方向 + 防环】DFS 到 hops 跳，path 内去重防环
    paths = []
